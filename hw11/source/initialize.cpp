...
void initialize(){
    ...
    while (infile1.getline(buf,sizeof(buf)))
    {
        ...
        else if (title == "cal_msd")
        {
            p = strtok(NULL,d);cal_msd = atof(p);
        }
        else if (title == "msd_interval")
        {
            p = strtok(NULL,d);msd_interval = atof(p);
        }
    ...
    delta_D = new double *[natoms];
    for (int i = 0; i < natoms; i++)
    {
       delta_D[i] = new double [3];
    }
}